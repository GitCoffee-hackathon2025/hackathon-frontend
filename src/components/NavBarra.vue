...
const rotaAtual = computed(() => infoRota.path.split('/')[1] || '')
...
const tamanhos = reactive<Record<string, string>>({})

const nomeVariavel = (padrao: string) => `--tamanho-${padrao === 'informacao' ? 'info' : padrao}`
    ...
    for (const key in refs) {
      ...
      if (el) {
        ...
        document.documentElement.style.setProperty(
          nomeVariavel(key === 'info' ? 'informacao' : key),
          value,
        )
      } else {
        // fallback seguro
        const fallback = '0px'
        tamanhos[key] = fallback
        document.documentElement.style.setProperty(
          nomeVariavel(key === 'info' ? 'informacao' : key),
          fallback,
        )
...
<template>
  <header>
    <nav
      :class="{ navExpandida: navExpandida }"
    ...
    >
      <ul>
        <li
          ...
          :key="index"
          :class="[classeDaRota(opcao.link), opcao.padrao]"
          :style="{ [nomeVariavel(opcao.padrao)]: tamanhos[opcao.refKey] ?? '0px' }"
        >
          <router-link :to="'/' + opcao.link">
            ...
            <p :ref="(el) => (refs[opcao.refKey] = el as HTMLElement | null)">{{ opcao.texto }}</p>
          </router-link>
        </li>
      </ul>
    </nav>
