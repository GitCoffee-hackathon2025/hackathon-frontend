  ...
  try {
    ...
    const res = await fetch(fetchUrl, {
      ...
      headers: {
        ...
        'Authorization': await AuthClient.getAccessToken()
      },
    ...
    if (!res.ok) {
      ...
      if (await user.recover(responseData)) {
        console.log('Tentando reenviar a ocorrência após recover...')
        return await sendOccurrence()
      }
